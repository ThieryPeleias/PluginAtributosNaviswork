...
- **Origin Reference Metadata Logging:** We log origin selection coordinates, merge depth, origin selection type (`Vertex`, `Element Center`, or `Default / Manual`), element name, and a unique identifier (native `InstanceGuid`, common properties GUID/Id/UniqueId/IfcGUID, or fallback hash code). If coordinate text boxes are edited manually, the selection metadata automatically reverts to `"Default / Manual"`.
